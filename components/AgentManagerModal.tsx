...
export const AgentManagerModal: React.FC<AgentManagerModalProps> = ({ isOpen, onClose, activeAgent, agentStatus }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
        ...
        <div className="relative w-full max-w-4xl theme-bg-main rounded-xl theme-border border shadow-2xl overflow-hidden flex flex-col h-[600px] animate-in zoom-in-95 duration-200">
            
            {/* Header */}
            <div className="p-6 theme-border border-b flex justify-between items-center theme-bg-sec">
                <div>
                ...
                <button onClick={onClose} className="p-2 hover:bg-black/10 dark:hover:bg-white/10 rounded-full text-gray-400 hover:theme-text transition-colors">
                    <X className="w-5 h-5" />
                </button>
            </div>

            {/* Main Content */}
            <div className="flex-1 p-8 grid grid-cols-1 md:grid-cols-2 gap-8 overflow-y-auto theme-bg-main">
                
                {/* Status Panel */}
                <div className="space-y-6">
                    <div className="theme-bg-sec rounded-lg p-6 theme-border border">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-xl font-mono theme-text">
                    ...
                    <div className="theme-bg-sec rounded-lg p-6 theme-border border">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 bg-black/5 dark:bg-white/5 rounded theme-border border">
                                <div className="text-xs text-gray-500 mb-1">Model</div>
                                <div className="text-sm font-bold theme-text">Claude 4.5 Opus</div>
                            </div>
                            <div className="p-3 bg-black/5 dark:bg-white/5 rounded theme-border border">
                        ...
                        {AGENTS.map(agent => (
                            <div 
                                ...
                                className={`flex items-center gap-4 p-4 rounded-lg border transition-all duration-300 ${
                                    ...
                                    : 'theme-bg-sec theme-border opacity-60'
                                }`}
                            >
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                                    activeAgent === agent.role ? 'bg-brand-primary text-white' : 'bg-black/10 dark:bg-white/10 text-gray-500'
                                }`}>
                                    <Bot className="w-5 h-5" />
                                </div>
                                <div className="flex-1">
                                    <div className="flex justify-between items-center">
                                        <h4 className={`font-bold ${activeAgent === agent.role ? 'theme-text' : 'text-gray-400'}`}>
                                            {agent.name}
                                        </h4>
                                        {activeAgent === agent.role && (
